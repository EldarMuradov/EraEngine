#include "game/tools/ecs_tools_system.h"
...
namespace era_engine
{
	...
	RTTR_REGISTRATION
	{
		...
		registration::class_<EcsToolsSystem>("EcsToolsSystem")
			.constructor<World*>()(policy::ctor::as_raw_ptr)(policy::ctor::as_raw_ptr, metadata("Tag", std::string("debug")))
			.method("update", &EcsToolsSystem::update)(metadata("update_group", update_types::GAMEPLAY_NORMAL));
	}

	EcsToolsSystem::EcsToolsSystem(World* _world)
		: System(_world)
	{

	}

	EcsToolsSystem::~EcsToolsSystem()
	{
	}

	void EcsToolsSystem::init()
	{
		renderer_holder_rc = world->add_root_component<RendererHolderRootComponent>();
		ASSERT(renderer_holder_rc != nullptr);
	}

	void EcsToolsSystem::update(float dt)
	{
		draw_world();
		draw_creation_popup();
	...
	void EcsToolsSystem::draw_creation_popup()
	{
		bool clicked = false;
	...
	void EcsToolsSystem::draw_childs(const Entity& entity)
	{
		...
		if (!childs.empty())
		{
			...
			for (; iter != end; ++iter)
			{
				...
				{
					if (ImGui::TreeNodeEx((void*)(uint32)(*iter), flags, "Unknown entity"))
					{
						if (ImGui::IsItemClicked())
						{
							set_selected_entity(child);
	...
	void EcsToolsSystem::update_selected_entity_ui_rotation()
	{
		if (selected_entity.is_valid())
		{
	...
	void EcsToolsSystem::set_selected_entity(const Entity& entity)
	{
