...
namespace era_engine::animation
{
	...
	void AnimationComponent::initialize(std::vector<animation_clip>& clips, size_t start_index)
	{
		...
		animation_blackboard startBlackboard = { &clips[start_index] };
	...
	void AnimationComponent::draw_current_skeleton(const ref<multi_mesh>& mesh, const trs& transform, ldr_render_pass* render_pass) const
	{
		...
		for (uint32 i = 0; i < numJoints; ++i)
		{
			...
			if (joint.parentID != INVALID_JOINT && !joint.ik)
			{
				...
				{
					*vertices++ = { current_global_transforms[joint.parentID].position, limbTypeColors[parentJoint.limbType] };
					*vertices++ = { current_global_transforms[i].position, limbTypeColors[parentJoint.limbType] };
				}
				else
				{
#if ROW_MAJOR
		...
		renderDebug<debug_unlit_line_pipeline::position_color>(trsToMat4(transform), vb, ib, vec4(1.f, 1.f, 1.f, 1.f), render_pass, true);
