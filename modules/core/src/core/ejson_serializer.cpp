...
namespace era_engine
{
	...
	void ejson_serializer::serializePrefab(eentity* entity)
	{
		...
		std::string filename = wstringToString(eproject::path) + "\\resources\\assets\\prefabs\\" + name + ".eprefab";
	...
	{
		std::string path = wstringToString(eproject::path) + "\\resources\\assets\\prefabs\\" + name.data() + ".eprefab";
	...
	const char* ejson_serializer::serializeScene(escene* scene, const char* name)
	{
		...
		std::string filename = wstringToString(eproject::path) + "\\resources\\assets\\scenes\\" + name + ".escene";
		std::ofstream o(filename.c_str());
		o << result << std::endl;
		o.close();
	...
	NODISCARD escene ejson_serializer::deserializeScene(std::string_view name)
	{
		...
		std::string path = wstringToString(eproject::path) + "\\resources\\assets\\scenes\\" + name.data() + ".escene";
