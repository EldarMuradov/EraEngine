...
internal static class DependencyResolver
{
    ...
    public static void RegisterSystemsWithReflection()
    {
        try
        {
            ...
            .AddJsonFile(Path.Combine(Project.EnginePath, "modules", "dotnet", "EraScriptingCore", "systems_injection.json"))
            .Build();
    ...
    public static void RegisterUserSystemsWithReflection(in Assembly assembly)
    {
        ...
        {
            var config = new ConfigurationBuilder()
            .AddJsonFile(Path.Combine(Project.Path, "resources", "assets", "scripts", Project.Name, Project.Name, "systems_injection.json"))
            .Build();
            var registration = config.GetSection("registrations").GetChildren();
            foreach (var reg in registration)
            {
