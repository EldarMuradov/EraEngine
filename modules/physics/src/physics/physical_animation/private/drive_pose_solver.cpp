...
namespace era_engine::physics
{
	...
	void DrivePoseSolver::solve_pose(float dt, bool solve_as_target) const
	{
		...
		for (const auto& limb_ptr : physical_animation_component->limbs)
		{
			...
			if (solve_as_target)
			{
				limb_data_component->adjusted_pose = limb_data_component->physics_pose;
			}
			else
			{
	...
	void DrivePoseSolver::solve_released_limb(PhysicalAnimationLimbComponent* limb_component, float dt) const
	{
		const PhysicalAnimationComponent* physical_animation_component = dynamic_cast<const PhysicalAnimationComponent*>(physical_animation_component_ptr.get());
