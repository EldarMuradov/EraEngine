...
namespace era_engine
{
    ...
    struct ERA_MOTION_MATCHING_API database
    {
    ...
    static void database_load(database& db, const char* filename)
    {
    ...
    static void database_save_matching_features(const database& db, const char* filename)
    {
    ...
    static int database_trajectory_index_clamp(database& db, int frame, int offset)
    {
        for (int i = 0; i < db.nranges(); i++)
        {
            if (frame >= db.range_starts(i) && frame < db.range_stops(i))
            {
                return clampi(frame + offset, db.range_starts(i), db.range_stops(i) - 1);
            }
        }
    ...
    static void forward_kinematics_velocity(
        Vec3& bone_position,
        Vec3& bone_velocity,
        Quat& bone_rotation,
    ...
    static void forward_kinematics_partial(
        slice1d<Vec3> global_bone_positions,
        slice1d<Quat> global_bone_rotations,
    ...
    static void forward_kinematics_velocity_partial(
        slice1d<Vec3> global_bone_positions,
    ...
    static void compute_bone_position_feature(database& db, int& offset, int bone, float weight = 1.0f)
    {
    ...
    static void compute_bone_velocity_feature(database& db, int& offset, int bone, float weight = 1.0f)
    {
        for (int i = 0; i < db.nframes(); i++)
        {
            Vec3 bone_position;
    ...
    static void compute_trajectory_position_feature(database& db, int& offset, float weight = 1.0f)
    {
        for (int i = 0; i < db.nframes(); i++)
        {
    ...
    // Same for direction
    static void compute_trajectory_direction_feature(database& db, int& offset, float weight = 1.0f)
    {
        for (int i = 0; i < db.nframes(); i++)
        {
            int t0 = database_trajectory_index_clamp(db, i, 20);
    ...
    static void database_build_bounds(database& db)
    {
    ...
    static void database_build_matching_features(
        database& db,
